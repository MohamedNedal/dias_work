...
def do_process(aia_file):
    """
    Process AIA image from lv1 to lv1.5
    """
    m = Map(aia_file)
    print(f'Upgrade AIA {channel}A {aia_file.split("/")[-1]} map to lv1.5 ..\n')
    aia_map_updated_pointing = update_pointing(m)
    ...
    aia_map_norm.plot_settings['norm'] = ImageNormalize(vmin=0, vmax=2e3, stretch=SqrtStretch())
    print('Image normalisation is finished')
