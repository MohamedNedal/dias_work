...
parser = argparse.ArgumentParser(description='Download SUVI images from GOES over a time range.')
parser.add_argument('-sdt', '--start_dt', required=True, type=str, help='Start datetime: "YYYY-MM-DD HH:MM[:SS]"')
parser.add_argument('-edt', '--end_dt', required=True, type=str, help='End datetime: "YYYY-MM-DD HH:MM[:SS]"')
parser.add_argument('-pb', '--passbands', required=True, nargs='+', type=int, help='AIA passbands as integers: 94 131 171 195 284 304 ...')
parser.add_argument('-sat_num', type=int, default=18, help='The GOES satellite number (e.g., 16 18)')

# Parse arguments
args = parser.parse_args()

passbands  = args.passbands
sat_number = args.sat_num
...
start_year, start_month, start_day     = str(start_dt).split(' ')[0].split('-')
start_hour, start_minute, start_second = str(start_dt).split(' ')[1].split(':')

end_year, end_month, end_day     = str(end_dt).split(' ')[0].split('-')
end_hour, end_minute, end_second = str(end_dt).split(' ')[1].split(':')


start_time = pd.Timestamp(f'{start_year}-{start_month}-{start_day} {start_hour}:{start_minute}:{start_second}')
end_time   = pd.Timestamp(f'{end_year}-{end_month}-{end_day} {end_hour}:{end_minute}:{end_second}')

for channel in passbands:
    os.makedirs(f'{data_dir}/SUVI/{start_year}{start_month}{start_day}/{channel}A', exist_ok=True)
    
    results = Fido.search(a.Time(start_dt.isoformat(), end_dt.isoformat()),
                          a.Instrument('suvi'),
                          a.goes.SatelliteNumber(sat_number),
                          a.Wavelength(channel*u.angstrom),
                          a.Level(2))
    
    print(f'\nDownloading SUVI {channel}A data for {start_year}-{start_month}-{start_day} ..\n')
    data = Fido.fetch(results, path=f'{data_dir}/SUVI/{start_year}{start_month}{start_day}/{channel}A/')
